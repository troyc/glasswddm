...
// Little function that makes sure the rectangle is specified correctly
void copy_rect(RECT *pRect, CONST RECT * pSrcRect)
{
    pRect->left = (pSrcRect->left < pSrcRect->right)  ? pSrcRect->left : pSrcRect->right;
    pRect->right = (pSrcRect->left < pSrcRect->right) ? pSrcRect->right : pSrcRect->left;
    pRect->top = (pSrcRect->top < pSrcRect->bottom)   ? pSrcRect->top : pSrcRect->bottom;
    pRect->bottom = (pSrcRect->top < pSrcRect->bottom)? pSrcRect->bottom : pSrcRect->top;
}
...
VOID GetPitches(_In_ CONST BLT_INFO* pBltInfo, _Out_ LONG* pPixelPitch, _Out_ LONG* pRowPitch)
{
    switch (pBltInfo->Rotation)
    {
        ...
        default:
        {
            BDD_LOG_ASSERTION("Invalid rotation (0x%I64x) specified", pBltInfo->Rotation);
            *pPixelPitch = 0;
            *pRowPitch = 0;
            return;
        }
    ...
    _In_reads_(NumRects) CONST RECT *pRects)
{
    ...
    {
        BDD_LOG_ERROR("Either dst (0x%p) or src (0x%p) bits encountered exception during access.", pDst->pBits, pSrc->pBits);
    }
}
